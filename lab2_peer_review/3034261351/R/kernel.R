...
GaussianKern <- function(x, h) {
  # Gaussian kernel function for use in nonparametric estimation.
  # Args:
  #  x: The point to evaluate the kernel
  #  h: The bandwidth of the kernel.
  # Returns:
  ...
  return( exp( -((x/h)^2)/2) / sqrt(2*pi) /h )
}

TriangularKern <- function(x, h){
  # Triangular kernel function for use in nonparametric estimation.
  # Args:
  #  x: The point to evaluate the kernel
  #  h: The bandwidth of the kernel.
  # Returns:
  ...
  return((abs(x/h)<0.5) * (2/h) * (1 - x*2/h))
}
