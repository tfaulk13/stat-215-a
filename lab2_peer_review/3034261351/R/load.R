...
loadMoteLocationData <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the 'mote-location-data' data file. 
  #         Path should be relative to the lab1.Rnw file. 
  # Returns:
  #   a data frame with five columns: ID, height, direction, distance, and tree.
...
loadLingData <- function(path = "data/"){
  # Arguments:
  # path: the path indicating the location of the 'lingData' data file. 
  #         Path should be relative to the lab2.Rmd file. 
  # Returns:
  #   a data frame with 47471 observations and 73 columns.
  
  # load in the txt file
  ling.data <- read.csv(paste0(path, "lingData.txt"), sep="", header = TRUE)
  return(ling.data)
}
...
loadLingLocation <- function(path = "data/"){
  ...
  # load in the txt file
  ling.location <- read.csv(paste0(path, "lingLocation.txt"), sep = "", header = TRUE)
  return(ling.location)
}