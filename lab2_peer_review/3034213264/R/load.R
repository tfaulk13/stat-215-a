...
loadMoteLocationData <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the `mote-location-data` file.
  #         Path should be relative to the lab1.Rnw file.
  # Returns:
  #   a data frame consisting of the mote locations
...
loadLingData <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the `lingData` txt file; path
  #         should be relative to the lab2.Rmd file
  # Returns:
  #   a data.frame with the columns: ID, CITY, STATE, ZIP, Q050-Q121, lat, long
  
  ling <- read.table(paste0(path, "lingData.txt"), header = T)
  return(ling)
}

loadLingLocationData <- function(path = "data/") {
  # Arguments:
  #   path: the path indicating the location of the `lingLocation` txt file; 
  #         path should be relative to the lab2.Rmd file
  # Returns:
  #   a data.frame with the ling categorical responses transformed into binary
  #   responses and binned into binned into 1 degree latitude x 1 degree 
  #   longitude squares
  
  ling_loc <- read.table(paste0(path, "lingLocation.txt"), header = T)
  return(ling_loc)
}
